...
che.dat = ReadMtx(
    mtx = '~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/che/GSE178318/suppl/GSE178318_matrix.mtx.gz',
    cells = '~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/che/GSE178318/suppl/GSE178318_barcodes.tsv.gz',
    features = '~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/che/GSE178318/suppl/GSE178318_genes.tsv.gz'
)

che.dat = che.dat[, gsub('.*_.*_(.*)', '\\1', colnames(che.dat)) != 'PBMC'] #remove pbmcs
unique(gsub('.*_.*_(.*)', '\\1', colnames(che.dat)))
...
macrophages.raw = che.dat[, colnames(che.dat) %in%
    colnames(macrophages)]

dim(macrophages.raw)

saveRDS(macrophages.raw, '~/work/ucl/bigdata/ucl.projects/macrophage/che/che.macrophages.RDS')

mac = readRDS('~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/che/che.macrophages.RDS')

test1
ncol(mac)

colnames(mac)
unique(gsub('.*_(.*)_(.*)', '\\1_\\2', colnames(mac)))

...
zheng.dir = '/camp/home/coultoa/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/zheng/GSE161277/suppl/carcinoma/'
g2 = list.files(zheng.dir, full.names = T, recursive = T, pattern = 'GSM')
zheng.df = 
    data.frame(
        full.path = g2,
...
zheng.df$samp.id = multi.str.split(zheng.df$file.name, '_', 1)
#filter blood and normal samples out
zheng.df = zheng.df %>%
    filter(!grepl('blood|normal', file.name))

zheng.split = split(zheng.df, zheng.df$samp.id)

zheng.dat = lapply(zheng.split, function(x){
    x2 = x %>%
...
sum(uulapply(zheng.dat, ncol))

sample.nums = paste0('s', 1:length(zheng.dat))
sample.nums

zheng.dat = Map(function(x, y){
    colnames(x) = paste0(y, '-', colnames(x))
    x
}, zheng.dat, sample.nums)

zheng.dat.comb = do.call(cbind, zheng.dat)
...
macrophages = GetAssayData(test1)[, Idents(test1) %in% c(cluster.means$cluster)]

macrophages.raw = zheng.dat.comb[, colnames(zheng.dat.comb) %in%
    colnames(macrophages)]

saveRDS(macrophages.raw, '~/work/ucl/bigdata/ucl.projects/macrophage/zheng/zheng.macrophages.RDS')
...
mac = readRDS('~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/zheng/zheng.macrophages.RDS')

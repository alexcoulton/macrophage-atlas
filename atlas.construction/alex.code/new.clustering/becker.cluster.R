...
g1 = '/camp/home/coultoa/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/becker/GSE201348/suppl/'
g2 = list.files(g1, full.names = T, recursive = T, pattern = 'GSM')
...
becker.df$samp.id = gsub('_[a-z]*.[a-z]*.gz', '', becker.df$file.name)

becker.split = split(becker.df, becker.df$samp.id)

becker.dat = lapply(becker.split, function(x){
    x2 = x %>%
        filter(grepl('matrix.mtx.gz', file.name))
    mtx.path = x2$full.path

    x3 = x %>%
        filter(grepl('barcodes.tsv.gz', file.name))
    cells.path = x3$full.path 

    x4 = x %>%
        filter(grepl('features.tsv.gz', file.name))
...
sort(names(becker.dat))

becker.dat = becker.dat[uulapply(crc.samps, function(x){
    grep(x, names(becker.dat))
})]

lapply(becker.dat, dim)
becker.samps = paste0('s', 1:length(becker.dat))

becker.dat = Map(function(x, y){
    colnames(x) = paste0(y, '-', colnames(x))
    x
}, becker.dat, becker.samps)

becker.dat.comb = do.call(cbind, becker.dat)
...
macrophages.raw = becker.dat.comb[, colnames(becker.dat.comb) %in%
    colnames(macrophages)]

dim(macrophages.raw)

saveRDS(macrophages.raw, '~/work/ucl/bigdata/ucl.projects/macrophage/becker/becker.macrophages.RDS')


mac = readRDS('~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/becker/becker.macrophages.RDS')

...
sade.dat = read.tsv('~/work/ucl/bigdata/ucl.projects/macrophage/sade/GSE120575/suppl/GSE120575_Sade_Feldman_melanoma_single_cells_TPM_GEO.txt')

rownames(sade.dat) = sade.dat$X

unique(sade.dat[1, ])
sade.dat = sade.dat[, -1]
sade.dat = as.matrix(sade.dat)

head(sade.dat)
sade.dat2 = Matrix(sade.dat, sparse = T)


h2(sade.dat2)

'CD68' %in% rownames(sade.dat2)
...
macrophages = GetAssayData(test1)[, Idents(test1) %in% c(0, 4, 10, 17, 33)]

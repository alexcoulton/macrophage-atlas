...
lu.dat = read.tsv('~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/lu/GSE149614/suppl/GSE149614_HCC.scRNAseq.S71915.count.txt')

h2(lu.dat)
unique(colnames(lu.dat))

samps1 = unique(gsub('_.*$', '', colnames(lu.dat)))
samps.raw = gsub('_.*$', '', colnames(lu.dat))
samps1 = samps1[grepl('T$', samps1)] # grab only primary tumour samples

lu.dat = lu.dat[, samps.raw %in% samps1]

lu.dat = as.matrix(lu.dat)
lu.dat2 = Matrix(lu.dat, sparse = T)

#saveRDS(lu.dat2, '~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/lu/lu.all.sparse.RDS')

#lu.dat2 = readRDS('~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/lu/lu.all.sparse.RDS')
...
macrophages.raw = lu.dat2[, colnames(lu.dat2) %in%
    colnames(macrophages)]

dim(macrophages.raw)

saveRDS(macrophages.raw, '~/work/ucl/bigdata/ucl.projects/macrophage/lu/lu.macrophages.RDS')

lu.mac = readRDS('~/work/ucl/bigdata/ucl.projects/macrophage/raw.study.data/lu/lu.macrophages.RDS')

unique(gsub('(.*)_.*', '\\1', colnames(lu.mac)))



#ncol(lu.dat2) - ncol(lu.mac)
#ncol(lu.mac)
